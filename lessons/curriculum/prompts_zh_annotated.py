# =============================================================================
# 【学习副本 / 带读注释版】mem0/configs/prompts.py
# -----------------------------------------------------------------------------
# 本文件是 mem0 源码 mem0/configs/prompts.py 的副本，仅用于学习。
# - 原始 prompt 英文正文保留（方便对照原文 + 避免破坏提示词语义）。
# - 所有 Python 注释、docstring 已翻译为中文。
# - 每个 prompt / 函数前补充了中文「带读」讲解（用 # ▶ 标记）。
#
# 文件整体结构（按出现顺序）：
#   1. MEMORY_ANSWER_PROMPT          —— 基于已检索记忆来回答用户问题的提示词
#   2. FACT_RETRIEVAL_PROMPT         —— 从对话中抽取「事实/偏好」的基础提示词（v1）
#   3. USER_MEMORY_EXTRACTION_PROMPT —— 只抽取「用户」相关事实的增强版
#   4. AGENT_MEMORY_EXTRACTION_PROMPT—— 只抽取「助手(AI)」相关事实的增强版
#   5. DEFAULT_UPDATE_MEMORY_PROMPT  —— 记忆「增/改/删/不变」四操作的决策提示词
#   6. PROCEDURAL_MEMORY_SYSTEM_PROMPT—— 把 Agent 执行历史压缩成「过程性记忆」摘要
#   7. get_update_memory_messages()  —— 组装「更新记忆」请求的函数
#   8. ADDITIVE_EXTRACTION_PROMPT    —— V3 增量抽取提示词（只做 ADD + 记忆关联）
#   9. AGENT_CONTEXT_SUFFIX          —— 把抽取主体切换成 AI agent 的视角后缀
#  10. generate_additive_extraction_prompt() 及其辅助函数 —— V3 用户侧 prompt 构建器
#
# 设计主线：mem0 的记忆写入分两阶段 ——
#   (A) 抽取(extraction)：从对话里提炼出「事实」列表（FACT/USER/AGENT/ADDITIVE 提示词）
#   (B) 整合(update)：把新事实和旧记忆比对，决定 ADD/UPDATE/DELETE/NONE
#       （DEFAULT_UPDATE_MEMORY_PROMPT + get_update_memory_messages）
#   V3(ADDITIVE) 是新一代方案：抽取阶段只做 ADD，并通过 linked_memory_ids 建立记忆图谱。
# =============================================================================

import json
from datetime import datetime, timezone

# ▶ MEMORY_ANSWER_PROMPT：检索式问答提示词。
#   用途：在 search/answer 流程里，把命中的记忆喂给 LLM，让它据此回答用户问题。
#   关键设计：① 只用记忆里的相关信息作答；② 即使没命中也不能说"找不到信息"，
#   要正常接住问题给出通用回答（避免冷冰冰的 "无结果"）；③ 答案简洁、直击问题。
...
# ▶ FACT_RETRIEVAL_PROMPT：事实抽取提示词（v1，最基础的一版）。
#   用途：mem0 写入记忆的第一步——把一段对话提炼成一组「事实/偏好」短句。
#   关键设计：
#     · 列出 7 类要记的信息（个人偏好、重要个人信息、计划意图、活动服务偏好、
#       健康养生、职业信息、杂项），给 LLM 明确的「记什么」边界。
#     · 用 few-shot 示例规定输出格式：固定为 {"facts": [...]}，无可记内容时返回空列表。
#     · 用 f-string 注入「今天的日期」，让 LLM 能正确处理"昨天/明天"等相对时间。
#     · 末尾要求「按用户输入语言记录事实」——中文输入就用中文记。
#   注意：这版从「用户和助手」消息里都抽取（与下面只抽用户/只抽助手的增强版不同）。
...
# USER_MEMORY_EXTRACTION_PROMPT - 基于 platform（线上平台）实现的增强版本
# ▶ 与 FACT_RETRIEVAL_PROMPT 的区别：只从「用户(user)」消息里抽事实，
#   明令禁止把「助手/系统」消息里的内容当作用户事实（并用 "会被惩罚" 强约束 LLM）。
#   场景：当 add() 指定了 user_id、只想沉淀「关于用户本人」的画像时使用。
...
# AGENT_MEMORY_EXTRACTION_PROMPT - 基于 platform（线上平台）实现的增强版本
# ▶ 与 USER 版镜像对称：只从「助手(assistant/AI)」消息里抽事实，
#   用来沉淀「关于 AI 助手自身」的画像（它的偏好、能力、性格、知识领域等）。
#   场景：当 add() 指定了 agent_id、想给某个 agent 建立稳定人设/能力记忆时使用。
...
# ▶ DEFAULT_UPDATE_MEMORY_PROMPT：记忆「整合(update)」阶段的核心提示词。
#   这是 mem0 写入两阶段里的第(B)步：把【新抽取的事实】与【已有记忆】逐条比对，
#   为每条决定四种操作之一：
#     · ADD    新信息，旧库里没有 → 生成新 id 加入
#     · UPDATE 同一主题但信息有变/更丰富 → 保留原 id，只更新文本
#     · DELETE 新信息与旧记忆矛盾 → 删除旧记忆
#     · NONE   已存在/无关 → 不动
#   关键约束：UPDATE/DELETE 必须沿用「输入里的原 id」，不能新造 id（保证记忆可追溯）。
#   提示词用大量 Old Memory / Retrieved facts / New Memory 三段式示例把规则讲透。
...
# ▶ PROCEDURAL_MEMORY_SYSTEM_PROMPT：过程性记忆(procedural memory)摘要提示词。
#   用途：当 Agent 跑了很多步后，把「执行历史」压缩成一份详尽摘要，让 agent 能
#   无歧义地接着干活。它和前面的「事实记忆」不同——记的是「做过什么/产出什么」。
#   关键设计：
#     · 顶部全局元数据：Task Objective(总目标) + Progress Status(进度百分比)。
#     · 按时间顺序编号记录每一步：Agent Action(动作) + Action Result(原样输出) +
#       Embedded Metadata(关键发现/导航历史/错误/当前上下文)。
#     · 反复强调「每一步的输出必须逐字保留、不得改写或概括」——因为后续要靠它复原结果。
...
# ▶ get_update_memory_messages：组装「记忆整合(update)」请求文本的函数。
#   入参：
#     · retrieved_old_memory_dict —— 当前已检索到的旧记忆（带 id）；可能为空。
#     · response_content          —— 上一步抽取出的「新事实」(JSON 字符串)。
#     · custom_update_memory_prompt —— 可选的自定义整合提示词；不传则用默认那条。
#   逻辑：
#     1. 没传自定义提示词 → 回落到 DEFAULT_UPDATE_MEMORY_PROMPT。
#     2. 旧记忆非空 → 把旧记忆拼进提示词让 LLM 在其基础上更新；为空 → 提示"当前记忆为空"。
#     3. 最终拼成一段完整 prompt：自定义规则 + 旧记忆 + 新事实 + 严格的 JSON 输出结构。
#   产出即第(B)步整合阶段真正发给 LLM 的 user 消息。
def get_update_memory_messages(retrieved_old_memory_dict, response_content, custom_update_memory_prompt=None):
    ...
    {{
        "memory" : [
            {{
                "id" : "<ID of the memory>",                # 更新/删除沿用已有 ID；新增则用新 ID
                "text" : "<Content of the memory>",         # 记忆的文本内容
                "event" : "<Operation to be performed>",    # 必须是 "ADD"、"UPDATE"、"DELETE" 或 "NONE"
                "old_memory" : "<Old memory content>"       # 仅当 event 为 "UPDATE" 时才需要
            }},
            ...
        ]
    }}
...
# ---------------------------------------------------------------------------
# V3 增量提取提示词（仅 ADD 操作，支持记忆关联）
# 移植自 platform/backend/shared/core/config/prompts.py
# ---------------------------------------------------------------------------
# ▶ ADDITIVE_EXTRACTION_PROMPT：新一代(V3)记忆抽取提示词，是本文件最长、最核心的一段。
#   与前面 v1 抽取(FACT/USER/AGENT)的根本区别：
#     · 旧版抽取后还要走 DEFAULT_UPDATE_MEMORY_PROMPT 做增/改/删；
#     · V3 抽取阶段「只做 ADD」，把"是否重复/是否更新"通过 linked_memory_ids（记忆关联）
#       和下游去重系统来处理，从而构建记忆「图谱」而非简单覆盖。
#   关键设计要点（读正文时重点关注）：
#     1. 同时从 user 和 assistant 消息抽取，但要正确归属(attributed_to)，并跳过"复述/客套"。
#     2. 输入分区：New Messages / Summary / Recently Extracted / Existing Memories /
#        Last k Messages / Observation Date / Current Date —— 各自用途讲得很细。
#     3. 时间锚点：所有"昨天/上周"等相对时间一律按 Observation Date(对话发生日)解析，
#        绝不能用 Current Date(系统今天)，否则记忆会随时间失真。
#     4. 质量标准：上下文要丰富(非原子化)、保留专有名词/数字/限定词、保持原意不泛化。
#     5. 完整性自检清单：防止"首话题主导"导致漏抽后半段/次要话题。
#     6. 输出严格 JSON：{"memory":[{id,text,attributed_to,linked_memory_ids?}]}。
...
# ▶ AGENT_CONTEXT_SUFFIX：把抽取「主体」从 user 切换成 AI agent 视角的追加后缀。
#   当记忆对象是某个 AI agent 时，会把这段拼到 ADDITIVE_EXTRACTION_PROMPT 之后，
#   要求 LLM 用"Agent 被告知/Agent 推荐/Agent 被配置为…"的句式来组织记忆文本，
#   但 attributed_to 仍反映原始来源(user 说的就标 user，agent 说的就标 assistant)。
...
# ---------------------------------------------------------------------------
# V3 提示词构建器 —— 构造用于「增量提取」的用户侧提示词
# 移植自 platform/backend/shared/core/utils/prompt_builder.py
# ---------------------------------------------------------------------------
# ▶ 下面这一组函数配合 ADDITIVE_EXTRACTION_PROMPT(系统提示词)使用：
#   系统提示词负责"规则"，这些 builder 负责把"真实数据"(摘要/历史/记忆/日期)
#   拼装成发给 LLM 的 user 侧提示词。前面几个下划线开头的是私有辅助函数。

# 历史消息单条内容的截断上限（字符数），防止把过长的旧消息整段塞进提示词。
PAST_MESSAGE_TRUNCATION_LIMIT = 300


def _truncate_content(text, limit=PAST_MESSAGE_TRUNCATION_LIMIT):
    """将文本截断到 limit 个字符，被截断时在末尾追加 '...'。"""
    if len(text) <= limit:
        return text
    return text[:limit] + "..."
...
def _format_conversation_history(messages):
    """将消息字典列表格式化为 'role: content' 形式的多行文本，并对内容做截断。"""
    if not messages:
        return ""
    result = ""
    for msg in messages:
...
def _resolve_dates(current_date=None, observation_date=None):
    """解析 current_date 与 observation_date，缺省时都用「今天」。"""
...
def generate_additive_extraction_prompt(
    ...
    *,
    ...
    """构建用于「增量(仅 ADD)抽取 + 记忆关联」的用户侧提示词。

    与 ADDITIVE_EXTRACTION_PROMPT(系统提示词)配套使用。
    LLM 只会产出 ADD 操作，并可附带 linked_memory_ids(关联到已有记忆的 id)。
    """
    # 解析日期：observation_date 缺省时取 timestamp，再缺省时回落到 current_date/今天
    current_date, observation_date = _resolve_dates(current_date, timestamp)

    # 按系统提示词约定的分区顺序，逐段拼装真实数据
    sections = []
    ...
    if custom_instructions:
        # 调用方传入的自定义指令，优先级最高，单独成段附在后面
        sections.append(f"## Custom Instructions\n{custom_instructions}")

    if use_input_language:
        # 多语言要求：让 LLM 用与输入相同的语言和书写系统输出（中文输入→中文记），
        # 不做翻译/转写；专有名词、品牌名保持原样；CJK 注意保持语气和省略主语的还原。
        sections.append(
    ...
    # 末尾追加 "# Output:" 触发标记，再把所有分区用空行连接成最终 user 提示词
