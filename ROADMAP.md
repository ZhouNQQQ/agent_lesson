...
5. **评测与可观测性** —— 三支柱、分层指标、LLM-as-Judge、trace/span、badcase 闭环。多份 JD 硬性要求。→ `lessons/05_evaluation/` + `knowledge/know_evaluation.md`、`know_observability.md` + `drills/05`
6. **MCP 与工具编排** —— 三原语、三角色、运行时发现、MCP vs FC。深圳/北京 JD 要求。→ `lessons/06_mcp/` + `knowledge/know_mcp.md`
7. **工程化（Harness/可靠性）** —— 长任务状态持久化与恢复、重试/降级/熔断、成本性能、多模型路由。决定能否接平台架构岗。→ `lessons/07_engineering/` + `knowledge/know_cost_reliability.md`、`know_model_routing.md` + `drills/06/07`
...
### P2 — 锦上添花
10. Prompt 工程体系化（静态 → 动态组装）、流式输出（SSE）、成本优化（缓存/分级路由）。

### 范围外（前置/自学，本仓不教）
- **前端/全栈栈（TypeScript/React/Node.js）**：部分全栈 AI-Native 岗（如 JD3）要求，但属通用前端技能，不在本仓 Agent 专项范围内，按需自学。
- **传统后端栈（Java/分布式/微服务/中间件）**：多数 JD 假设已具备，本仓不教（但工程化模块的状态管理/可靠性会用到这些心智）。
...
| 工具与协议（MCP/FC） | FC 的 JSON Schema、MCP 三原语、工具编排 | | | P1 | ⬜ |
| 评测与可观测性 | 三支柱、分层指标、LLM-as-Judge、trace、badcase 闭环 | | | P1 | ⬜ |
| 工程化（Harness/可靠性） | 状态持久化/恢复、重试降级熔断、成本性能、多模型路由 | | | P1 | ⬜ |
| 安全 | 直接/间接注入、越狱、沙箱、权限审计、HITL | | | P1 | ⬜ |
| 系统设计大题 | 多 Agent、可观测性、研发流水线 | | | P1 | ⬜ |
...
| 5 | LLM-as-Judge 评分器（pairwise + 位置消偏 + 共识） | `drills/05_llm_as_judge.py` | ⬜ |
| 6 | 多模型级联路由（小模型先答 + 置信门控升级大模型） | `drills/06_model_router.py` | ⬜ |
| 7 | 可靠性三件套（指数退避重试 + 降级 + 熔断器） | `drills/07_retry_fallback.py` | ⬜ |

**建议顺序**：先 ReAct 默写结业 → RAG（P0 大头）→ 混合记忆召回 → 工具路由 → LLM-as-Judge → 可靠性三件套 → 多模型路由。
...
| 评测三支柱 | `knowledge/know_evaluation.md`、`lessons/05_evaluation/` | 三支柱各管什么？为什么检索/生成要分开测？离线好上线差为什么？ |
| 可观测性 | `knowledge/know_observability.md`、`drills/05` | trace/span 怎么组织？为什么要存全量快照？badcase 怎么闭环？ |
| MCP | `knowledge/know_mcp.md`、`lessons/06_mcp/` | 三原语？运行时发现两层？MCP vs Function Calling？ |
| 工程化/可靠性 | `knowledge/know_cost_reliability.md`、`drills/07` | 长任务崩了怎么恢复？重试/降级/熔断分工？fail-open vs closed？ |
| 多模型路由 | `knowledge/know_model_routing.md`、`drills/06` | 级联怎么判断升级？路由和降级区别？怎么省钱？ |
| Agent 安全 | `knowledge/know_agent_security.md`、`lessons/08_security/` | 直接 vs 间接注入？为什么防不死？执行代码怎么沙箱？HITL 卡哪些？ |
