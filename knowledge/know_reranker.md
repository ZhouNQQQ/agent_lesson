...
### ❌ 错误1：把 Over-fetch 倍数说成"拍脑袋的经验值"

**错误表述**："4 倍就是个经验值，没什么依据"
**正确理解（分两层，别混）**：
- **原理上**：over-fetch 取多大，合理的标定方法是看 **recall@k 评估曲线的边际收益拐点**——2 倍时 recall≈65%（漏太多）、4 倍≈90%（拐点）、8 倍≈95%（只多 5% 但成本翻倍，收益递减）。严谨做法是用真实数据测这条曲线来定倍数。
- **工程上**：但 mem0 源码里的 4 倍（`max(limit×4, 60)`）是**工程经验缺省值，并不是用 recall@k 实验标定出来的**（见 `materials/02_mem0_search_pipeline.md`）。
- **结论**：方法论上"该用 recall@k 曲线定"，工业落地时 mem0 取了个"经验缺省值"——两者不矛盾。说"4 倍是经验值"没错，错在以为"经验值=没有更严谨的定法"。
**来源**：K.5 口述追问
